"""Export the dashboard ``kpi_*`` tables to CSV for the self-contained web dashboard.

Run this to dump every KPI table from both warehouses to ``dashboard/exports/*.csv``;
``build_web_dashboard.py`` then embeds those CSVs into ``web/index.html`` so the page
works fully offline (no server, no ODBC).
...
DATABASES = ["bigdata_etl", "bigdata_elt"]   # ETL warehouse, ELT warehouse
